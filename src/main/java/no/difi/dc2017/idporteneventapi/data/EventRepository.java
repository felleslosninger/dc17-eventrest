...
public interface EventRepository extends JpaRepository<Event, Long>{
    ...
    @Query(value = "select auth_type.id, auth_type.value, count(auth_type.id) from (auth_type inner join (select auth_type from event where ssn = ? limit 50000) as ev on ev.auth_type = auth_type.id) group by auth_type.id", nativeQuery = true)
